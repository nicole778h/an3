...
export interface ItemsState {
  ...
  saveItem?: SaveItemFn;
  getItems?: (page: number, limit: number) => Promise<{ items: ItemProps[]; totalPages: number }>;
  currentPage: number;
  pageSize: number;
  totalPages: number;
...
const reducer: (state: ItemsState, action: ActionProps) => ItemsState = (state, { type, payload }) => {
  switch (type) {
    ...
    case SAVE_ITEM_SUCCEEDED:
      ...
      if (index === -1) {
        items.unshift(item); // Adaugă item-ul la început dacă nu există
      } else {
        items[index] = item; // Înlocuiește item-ul existent
      }
...
export const ItemProvider: React.FC<ItemProviderProps> = ({ children }) => {
  ...
  const fetchAndSetItems = useCallback(async (page: number, limit: number): Promise<{ items: ItemProps[]; totalPages: number }> => {
    ...
    try {
      ...
      const { items, totalPages } = await fetchItems(page, limit);
      log('fetchAndSetItems succeeded', items);
      if (!canceled) {
        dispatch({ type: FETCH_ITEMS_SUCCEEDED, payload: { items, totalPages, currentPage: page } });
        localStorage.setItem('fetchedData', JSON.stringify(items)); // Salvează în localStorage
      }
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      dispatch({ type: FETCH_ITEMS_SUCCEEDED, payload: { items: parsedData, totalPages: 1, currentPage: 1 } });
    } else {
